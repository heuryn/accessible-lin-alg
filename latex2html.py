...
def convert(tex, filename):
    prompt = (
        "You are a converter that transforms LaTeX Beamer slides into clean HTML.\n"
        "Use MathJax for math. Assume all \\includegraphics refer to images in the folder 'images'.\n"
        "Convert this LaTeX content to valid standalone HTML:\n\n"
        f"{tex}\n\n"
        "Output only valid HTML5. Do not explain anything."
    )
    
    print(f"[{filename}] Tokenizing input...")
